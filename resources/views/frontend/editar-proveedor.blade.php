...
<head>
    ...
    <title>Editar Proveedor - Technova</title>
    ...
    <style>
        ...
        .form-editar-proveedor h2 {
            color: #007acc;
            margin-bottom: 20px;
            text-align: center;
        }
        .form-editar-proveedor input[type="text"],
        .form-editar-proveedor input[type="email"],
        .form-editar-proveedor select {
            width: 100%;
            padding: 12px;
            margin-bottom: 15px;
        ...
        .form-editar-proveedor input:focus,
        .form-editar-proveedor select:focus {
            outline: none;
            border-color: #007acc;
        }
        .btn-guardar {
...
<body>
    ...
    <div class="dashboard-wrapper">
        ...
        <main class="main-content">
            <h1><i class='bx bx-edit'></i> Editar Proveedor</h1>
            ...
            <div class="form-editar-proveedor">
                <h2><i class='bx bx-user-edit'></i> Editar Información del Proveedor</h2>
                <form action="{{ route('proveedores.update', $proveedor->ID_Proveedor) }}" method="POST">
                    @csrf
                    @method('PUT')
                    
                    <input type="text" name="Identificacion" value="{{ old('Identificacion', $proveedor->Identificacion) }}" placeholder="Identificación (CC, NIT, etc.)" required />
                    
                    <input type="text" name="Nombre" value="{{ old('Nombre', $proveedor->Nombre) }}" placeholder="Nombre completo del proveedor" required />
                    
                    <input type="text" name="Empresa" value="{{ old('Empresa', $proveedor->Empresa) }}" placeholder="Nombre de la empresa" required />
                    
                    <input type="text" name="Telefono" value="{{ old('Telefono', $proveedor->Telefono) }}" placeholder="Teléfono" required />
                    
                    <input type="email" name="Correo" value="{{ old('Correo', $proveedor->Correo) }}" placeholder="Correo electrónico" required />
                    
                    <select name="ID_producto">
                        <option value="">Seleccionar Producto (Opcional)</option>
                        @foreach($productos as $producto)
                            <option value="{{ $producto->ID_Producto }}" {{ old('ID_producto', $proveedor->ID_producto) == $producto->ID_Producto ? 'selected' : '' }}>
